...
export const getTicksData = async (id) => {
  let { data, error } = await supabase.rpc('get_single_user_ticks', {
    input_id: id,
  });
  if (data) {
    if (data.length > 0) {
...
export const removeImage = async (image, bucket) => {
  ...
  try {
    const { error } = await supabase.storage.from(bucket).remove(image);
...
export const getFollowedUserTicks = async (user_id) => {
  try {
    let { data, error } = await supabase.rpc('get_followed_user_ticks', {
      input_id: user_id,
    });
    if (error) {
      throw error;
...
export const downloadAttractionsData = async (user_id) => {
  let { data, error } = await supabase.rpc('get_attractions_data', {
    input_id: user_id,
  });
  if (data) {
    return data;
...
export async function removeTick(id, attraction_id) {
  ...
  if (error) {
    Alert.alert(
      'Tick not removed from database',
      'Please reset attractions in settings and try again'
    );
  }
}